...
def format_mcqs(questions_text):
    ...
    questions = re.split(r'(?=\d+\.|\bQuestion\s+\d+:)', questions_text)
    
    questions = [q.strip() for q in questions if q.strip()]
    
    for question in questions:
        if not re.search(r'^\d+\.|\bQuestion\s+\d+:', question):
            continue
            
        html_output += '<div class="border-2 border-gray-300 rounded-lg p-4 mb-4 shadow-sm">'
        
        question_match = re.match(r'((?:\d+\.|\bQuestion\s+\d+:).*?)(?=[A-D]\.|\([A-D]\))', question, re.DOTALL)
        
        correct_answer = None
        answer_match = re.search(r'(?:Answer|Correct Answer|Correct):\s*([A-D])', question, re.IGNORECASE)
        if answer_match:
            correct_answer = answer_match.group(1)
            
            question = re.sub(r'(?:Answer|Correct Answer|Correct):\s*[A-D].*?$', '', question, flags=re.DOTALL | re.IGNORECASE).strip()
        
        if question_match:
            question_text = question_match.group(1).strip()
            html_output += f'<p class="font-bold mb-2">{question_text}</p>'
            
            options_text = question[len(question_match.group(1)):].strip()
            options = re.findall(r'([A-D]\.|\([A-D]\))(.*?)(?=[A-D]\.|\([A-D]\)|Answer:|Correct Answer:|Correct:|$)', options_text, re.DOTALL | re.IGNORECASE)
            
            html_output += '<div class="pl-4">'
            for option_label, option_text in options:
                option_letter = re.search(r'[A-D]', option_label).group(0)
                html_output += f'<p class="mb-1">{option_label}{option_text.strip()}</p>'
            html_output += '</div>'
            ...
            if correct_answer:
                ...
                <div class="mt-3">
                    ...
                    <p class="hidden mt-2 font-bold text-green-600">Correct Answer: {correct_answer}</p>
                </div>
                '''
        else:
            html_output += f'<p>{question}</p>'
